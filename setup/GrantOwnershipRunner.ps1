...
function Grant-Ownership{
    ...
    try{
        ...
        If( $PSBoundParameters.ContainsKey('ObjectType') -eq $True ){
            Write-Host "[TAKEOWN] " -f Blue -NoNewLine
            Write-Host " ObjectType $ObjectType"
        }else{
            Write-Host "[TAKEOWN] " -f Blue -NoNewLine
            Write-Host " ObjectType file"
            $ObjectType = 'file'
        }     
        ...
        Write-Host "[TAKEOWN] " -f Blue -NoNewLine
        Write-Host "$ObjectType : $Path. Recurse $Recurse"
        ...
        if ($ProcessExitCode -ne 0) {
            Write-Host -ForegroundColor DarkRed "[TAKEOWN] " -NoNewline
            Write-Host " ERROR takeown exited with status code $ProcessExitCode"
            throw " ERROR takeown exited with status code $ProcessExitCode"
        }
        Write-Host "[TAKEOWN] " -f DarkGreen -NoNewLine
...
If( $PSBoundParameters.ContainsKey('Recurse') -eq $false ){
    $Recurse = $False
}
Grant-Ownership -Path $Path -ObjectType $ObjectType -Recurse:$Recurse
