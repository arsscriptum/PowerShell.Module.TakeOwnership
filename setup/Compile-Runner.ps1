...
try{
    ...
    $RunnerScriptPath = Join-Path $SetupPath 'GrantOwnershipRunner.ps1'
    ...
    Invoke-ps2exe -inputFile $RunnerScriptPath -outputFile "$RunnerPath" -iconFile "$IconPath" -requireAdmin

    Write-ChannelResult "SUCCESS!"  
    Write-Host "Copy-Item $RunnerPath ==> $Env:ToolsRoot"

    Copy-Item $RunnerPath $Env:ToolsRoot
}catch {
