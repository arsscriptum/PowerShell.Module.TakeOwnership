...
function Uninstall-ContextualMenu
{
    ...
    $RegistryPath = 'HKCR:\Directory\shell\GrantOwnership'
...
function Install-ContextualMenu{
    ...
    try{
        ...
        Write-ChannelMessage  "====================================="
        $RegistryRootPath = 'HKCR:\Directory\shell\GrantOwnership'
        $RegistryChildPath = "$RegistryRootPath\command"

        $RootPath = (Resolve-Path "$PSScriptRoot\..").Path
        $BinPath = (Resolve-Path "$Env:ToolsRoot\").Path
        $ImgPath = Join-Path $RootPath 'img'
        $IconPath = Join-Path $ImgPath 'Administrator.ico'
        $RunnerPath = Join-Path $BinPath 'GrantOwnership.exe'
        ...
        $Cmd = '"' + $RunnerPath + '"'
        $Cmd += ' -Path "%1" -ObjectType file -Recurse'
        New-Item $RegistryChildPath -Force -Value "$Cmd"
        Write-ChannelMessage  "Cmd $$Cmd"

        if(-not(Test-Path $IconPath)){
            $IconPath = [Environment]::GetFolderPath("MyPictures")
            $IconPath = Join-Path $IconPath 'go.png'
            Write-ChannelMessage  "IconPath $IconPath UDPATE"
        }
        
        New-ItemProperty $RegistryRootPath -Name 'Icon' -Value  "$IconPath" | Out-Null
        New-ItemProperty $RegistryRootPath -Name 'MUIVerb' -Value 'Grant Ownership' | Out-Null
        Remove-PSDrive -Name HKCR
    } catch {
